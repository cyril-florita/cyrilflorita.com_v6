...
- DOM order is the "All" order: tiles with motion thumbnails first (Hunger Action Month and GTY v8 play looping videos via `GridVideo`, GTY Dashboard crossfades its screens via `GridSlideshow`), then the other Design & Development projects (35-Day Generosity Challenge, Giving Tuesday, Truth Matters, Grace Stream, App Landing, GTY v9, The Study Bible App), then branding and campaigns, then illustration; the individual graphics follow in a random order. Filter order: All, Design & Development (`fil-uix` — class name kept for history), Marketing & Branding; "All" is the default, and a filter restored on return switches to All if it would hide the project being returned to. **Each filter sorts by its own order** — `data-order-uix` for Design & Development, `data-order-brand` for Marketing & Branding (case studies only; graphics sort after, in their random order) — chosen so every tile a filter keeps lands in a different position than in "All". That guaranteed movement (plus the `.cyril-is-filtering` dip) is how visitors see the filter working; keep it when reordering or adding projects: no tile should keep its "All" position in a filtered view (current scheme: within each filter, the motion tiles rotate among themselves and stay first, and the rest rotate among themselves).
...
`components/case/CaseStudy.js` holds the editorial project-page building blocks — `CaseHero` (eyebrow — category + descriptor, no dates —, big left-aligned title, summary, facts row, full-bleed image that settles from a zoom on scroll), `CaseLayout` (sticky left rail — the numbered section index that highlights the current section, with a "Back to All Work" button under it — + content column; the rail is hidden ≤1200px), `CaseSection`, `CaseFigure` (lightbox image; `ratio` crops very tall images to a framed top preview; `frame="browser"` + optional `url` puts a screenshot in the browser-window frame), `CaseVideo` (desktop recordings in a browser-window frame, `url` fills its address bar; `size="phone"` uses a phone frame instead), `CaseGrid` (`two` / `offset`), `CaseStats`, `CaseQuote`, `CaseNext` ("Next project" band, plus a "Back to All Work" button shown only ≤1200px where the rail is hidden; both back buttons are `BackToAllWork`). Styles are the "case study" block at the end of `_components.scss`, all scoped to `.cyril-case-*`. All 17 project pages use it. The section index only renders with 3+ sections (otherwise the left column is an empty margin and section numbers are hidden). Conventions: no dates in the hero/facts (older work shouldn't read as dated), facts only when the page text supports them, captions from original labels/alt/filenames, and `CaseNext` points at the next project in `PortfolioIsotope`'s DOM order (the last wraps back to the first, `/hunger-action-month`) — keep that chain in sync if the grid order changes. Identity projects can add a brand guide from `components/case/BrandGuide.js` (`BrandLogos`, `BrandClearSpace`, `BrandSwatches`, `BrandLettering` — crops of the logo art via `BrandCrop`, `BrandUsage` do/don't tiles; styles in the "brand guide" block at the end of `_components.scss`); used on `/patricia-macarthur-pastoral-care-fund`, `/the-study-bible-app-logo`, `/grace-stream` and `/truth-matters`. New project materials are staged in `new-work/` (git-ignored; originals stay there) and exported as optimized JPGs to `public/img/portfolio/` with a `chf-`-style prefix. The old project-page-only styles (`.cyril-project-page .cyril-top-banner`, `.cyril-project-content`) are now unused; `.cyril-top-banner` itself is still used by the My Work heading on `/`.
