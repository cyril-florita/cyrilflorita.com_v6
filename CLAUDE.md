...
`components/case/CaseStudy.js` holds the editorial project-page building blocks — `CaseHero` (eyebrow — category + descriptor, no dates —, big left-aligned title, summary, facts row, full-bleed image that settles from a zoom on scroll), `CaseLayout` (sticky left rail — the numbered section index that highlights the current section, with a "Back to All Work" button under it — + content column; the rail is hidden ≤1200px), `CaseSection`, `CaseFigure` (lightbox image; `ratio` crops very tall images to a framed top preview), `CaseVideo`, `CaseGrid` (`two` / `offset`), `CaseStats`, `CaseQuote`, `CaseNext` ("Next project" band, plus a "Back to All Work" button shown only ≤1200px where the rail is hidden; both back buttons are `BackToAllWork`). Styles are the "case study" block at the end of `_components.scss`, all scoped to `.cyril-case-*`. All 14 project pages use it. The section index only renders with 3+ sections (otherwise the left column is an empty margin and section numbers are hidden). Conventions: no dates in the hero/facts (older work shouldn't read as dated), facts only when the page text supports them, captions from original labels/alt/filenames, and `CaseNext` points at the next project in `PortfolioIsotope`'s DOM order (the last wraps to `/gty_v9`) — keep that chain in sync if the grid order changes. Identity projects can add a brand guide from `components/case/BrandGuide.js` (`BrandLogos`, `BrandClearSpace`, `BrandSwatches`, `BrandLettering` — crops of the logo art via `BrandCrop`, `BrandUsage` do/don't tiles; styles in the "brand guide" block at the end of `_components.scss`); used on `/patricia-macarthur-pastoral-care-fund`, `/the-study-bible-app-logo`, `/grace-stream` and `/truth-matters`. The old project-page-only styles (`.cyril-project-page .cyril-top-banner`, `.cyril-project-content`) are now unused; `.cyril-top-banner` itself is still used by the My Work heading on `/`.
